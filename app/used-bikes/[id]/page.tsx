...
import { getBikeBySlugOrId, getPublicStockBikes, type PublicStockDetailBike } from "@/lib/stock";
...
function makeSpecs(bike:PublicStockDetailBike){
  ...
  const stripCandidates:SpecItem[]=[
    ...
    {label:"CO₂",value:find("CO2","CO2 Emissions"),icon:"leaf"},{label:"Year",value:String(bike.year||""),icon:"calendar"},{label:"Road tax",value:find("Road Tax","Tax"),icon:"tax"},
...
const descriptionHeadings=[
  ...
  {key:"included",title:"What’s Included Before Delivery",aliases:["What's Included Before Delivery","What’s Included Before Delivery","Included Before Delivery"]},
...
function parseDescription(text:string,attention:string):DescriptionSection[]{
  ...
  for(const raw of prepared.split(/\n+/)){const line=raw.trim();if(!line)continue;const marker=line.match(/^@@(.+)@@$/);if(marker){push();const key=marker[1];current={key,title:definitions.get(key)??"Details",content:[]};continue}current.content.push(line)}push();
  if(attention&&!sections.some(section=>section.key==="headline"))sections.unshift({key:"headline",title:"The headline",content:[attention]});
...
function DescriptionContent({text,attention,structured}:{text:string;attention:string;structured:Record<string,unknown>}){
  const structuredSections:DescriptionSection[]=descriptionHeadings.map(item=>({key:item.key,title:item.title,content:typeof structured[item.key==="headline"?"advert_headline":item.key==="intro"?"intro_description":item.key==="details"?"key_details":item.key==="extras"?"fitted_extras":item.key==="preparation"?"preparation_work":item.key==="included"?"included_before_delivery":item.key==="why"?"why_buy_from_yesmoto":"finance_options"]==="string"?(structured[item.key==="headline"?"advert_headline":item.key==="intro"?"intro_description":item.key==="details"?"key_details":item.key==="extras"?"fitted_extras":item.key==="preparation"?"preparation_work":item.key==="included"?"included_before_delivery":item.key==="why"?"why_buy_from_yesmoto":"finance_options"] as string).replace(/[•●▪◦]\s*/g,"\n• ").split(/\n+/).map(line=>line.trim()).filter(Boolean):[]})).filter(section=>section.content.length);
  const sections=structuredSections.length?structuredSections:parseDescription(text,attention);
  return <div className="advert-sections">{sections.map(section=>{const bullets=section.content.filter(line=>/^[•*\-✓✔]/.test(line));const prose=section.content.filter(line=>!/^[•*\-✓✔]/.test(line));return <article className={`advert-section ${section.key}`} key={`${section.key}-${section.title}`}><header><span>{section.key==="headline"?"YESMOTO":"ADVERT DETAILS"}</span><h3>{section.title}</h3></header><div>{prose.map((line,index)=>{const pieces=line.length>420?line.split(/(?<=[.!?])\s+(?=[A-Z])/).reduce<string[]>((groups,sentence)=>{const last=groups.at(-1)??"";if(last.length<240){groups[groups.length-1]=`${last} ${sentence}`.trim()}else groups.push(sentence);return groups},[""]).filter(Boolean):[line];return pieces.map((piece,pieceIndex)=><p key={`${index}-${pieceIndex}`}>{piece}</p>)})}{bullets.length>0&&<ul>{bullets.map((line,index)=><li key={index}>{line.replace(/^[•*\-✓✔]\s*/,"")}</li>)}</ul>}</div></article>})}</div>;
}
...
export default async function Detail({params}:{params:Promise<{id:string}>}){
  ...
  return <main className="vehicle-detail premium-advert">
    ...
    <section className="vehicle-enquiry" id="enquiry"><div className="wide vehicle-wrap"><div className="enquiry-intro"><p className="vehicle-kicker">ASK ABOUT THIS MOTORCYCLE</p><h2>Let’s talk about your next bike</h2><p>Ask for another photograph, arrange a callback, or discuss finance, delivery and part exchange with someone who knows motorcycles.</p><div><a href={phoneHref}>{dealership.phone}</a><span>{dealership.openingHours}</span></div></div><VehicleEnquiryForm bike={title} whatsapp={whatsapp}/></div></section>
    <section className="vehicle-related wide vehicle-wrap"><div className="section-title"><h2>YOU MAY ALSO LIKE</h2><Link href="/stock">View all bikes</Link></div><div className="bike-grid">{related.filter(item=>item.id!==bike.id).slice(0,3).map(item=><BikeCard bike={item} key={item.id}/>)}</div></section>
  </main>;
}
