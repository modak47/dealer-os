...
function PublicStockCard({ bike }: { bike: PublicStockBike }) {
  ...
  return <Link prefetch href={`/used-bikes/${bike.slug}`} onClick={() => setOpening(true)} className={`public-stock-card ${opening ? "opening" : ""}`}><div className="public-stock-image"><StockImage bike={bike} /><span className={bike.status === "Reserved" ? "reserved" : ""}>{bike.status}</span><b>{bike.photoReady ? `${bike.imageUrls.length} photos` : "Photos coming soon"}</b>{opening && <span className="bike-opening"><i />Opening bike...</span>}</div><div className="public-stock-copy"><p>{bike.year || "Year unavailable"}</p><h2>{bike.make} {bike.model}</h2>{bike.variant && <h3>{bike.variant}</h3>}<div className="public-stock-specs">{specs.map((spec) => <span key={spec}>{spec}</span>)}</div><div className="public-stock-finance"><span>Finance from approximately £{bike.monthly}/month</span><b>{bike.status === "Reserved" ? "Reserved" : bike.reserveEnabled ? "Reserve online" : "Enquire first"}</b></div><div className="public-stock-action"><strong>{money(bike.price)}</strong><b>View motorcycle →</b></div></div></Link>;
}
