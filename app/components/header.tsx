...
export function Header(){
  ...
  if(pathname?.startsWith("/dealer-portal")||pathname==="/dealer-login"||pathname==="/dealer-login-v4-preview")return null;
  ...
  return <>
    ...
    <header><div className="wide header-row">
      ...
      <div className="head-actions"><Link href="/dealer-login" className="staff-login" onClick={closeMenus}>Dealer Portal</Link><Link href="/admin" className="staff-login" onClick={closeMenus}>Staff Login</Link><Link href="/used-bikes" onClick={closeMenus} aria-label="Search"><Search/></Link><Link href="/used-bikes?favourites=true" onClick={closeMenus} aria-label="Favourites"><Heart/></Link><a href={phoneHref} className="phone"><Phone/><span><b>{dealership.phone}</b><small>{dealership.openingHours}</small></span></a></div>
      <div className={`mobile-nav ${mobileOpen?"open":""}`}><button type="button" onClick={()=>setMobileOpen(open=>!open)} aria-label="Open navigation" aria-expanded={mobileOpen}><Menu/></button>{mobileOpen&&<div><Link href="/dealer-login" className="mobile-staff-login" onClick={closeMenus}>Dealer Portal<span>→</span></Link><Link href="/admin" className="mobile-staff-login" onClick={closeMenus}>Staff Login<span>→</span></Link>{nav.map(([name,href])=><Link href={href} onClick={closeMenus} key={name}>{name}<span>→</span></Link>)}<p>MORE</p>{moreLinks.map(item=><Link href={item.href} onClick={closeMenus} key={item.href}>{item.name}<span>→</span></Link>)}</div>}</div>
    </div></header>
  </>;
}
