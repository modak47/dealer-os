...
export default async function Dashboard() {
  ...
  return <AdminPage title={`Welcome back, ${identity?.firstName ?? "team"}`} sub={`Here's what's happening across ${dealership.dealerName} today.`} hint={`Signed in as: ${identity?.email ?? "authenticated user"}`} actions={<div className="quick-actions"><Link href="/admin/stock/book-in">Book Into Stock</Link><Link href="/admin/leads">View Leads</Link><Link href="/" target="_blank">View Website</Link><Link href="/admin/sales-channels">Sales Channels</Link></div>}>
