...
export async function serveLeadImage(request: Request) {
  ...
  const upstream = await fetch(imageUrl, { cache: "no-store", redirect: "error", signal: AbortSignal.timeout(15000) });
  ...
  if (sourceBuffer.length > 15 * 1024 * 1024) return NextResponse.json({ error: "Image is too large." }, { status: 413 });
