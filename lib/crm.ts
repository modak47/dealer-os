...
export async function getCrmLeads(status=""):Promise<CrmResult<CrmLead[]>>{
  let request=getSupabaseAdmin().from("crm_leads").select("*,customer:crm_customers(id,first_name,last_name,email,phone),bike:stock_bikes(id,make,model,registration)").order("updated_at",{ascending:false}).limit(100);
...
export async function getCrmDashboard(){
  ...
  const [newLeads,enquiries,reservations,activities,overdue,finance,sales]=await Promise.all([
    db.from("crm_leads").select("id",{count:"exact",head:true}).eq("status","New"),db.from("crm_enquiries").select("id",{count:"exact",head:true}).gte("created_at",today.toISOString()),
    db.from("crm_reservations").select("id",{count:"exact",head:true}).in("status",["Active","Deposit Taken"]),db.from("crm_activities").select("id",{count:"exact",head:true}).eq("status","Open").gte("due_at",today.toISOString()).lt("due_at",new Date(today.getTime()+86400000).toISOString()),
    ...
    db.from("crm_sales").select("id",{count:"exact",head:true}).gte("created_at",month).neq("status","Cancelled")]);
...
export async function searchCrm(query:string){const term=query.trim();if(!term)return {customers:[] as CrmCustomer[],leads:[] as CrmLead[],stock:[] as {id:string;make:string|null;model:string|null;registration:string|null;vin:string|null;stock_number:string|null}[],reservations:[] as CrmReservation[],sales:[] as {id:string;status:string;customer?:{first_name:string;last_name:string}|null;bike?:{make:string|null;model:string|null}|null}[]};const db=getSupabaseAdmin(),pattern=`%${term}%`;const [customers,leads,stock,reservations,sales]=await Promise.all([db.from("crm_customers").select("*").or(`first_name.ilike.${pattern},last_name.ilike.${pattern},email.ilike.${pattern},phone.ilike.${pattern},postcode.ilike.${pattern}`).limit(20),db.from("crm_leads").select("*,customer:crm_customers(id,first_name,last_name,email,phone),bike:stock_bikes(id,make,model,registration)").or(`preferred_bike_notes.ilike.${pattern},notes.ilike.${pattern}`).limit(20),db.from("stock_bikes").select("id,make,model,registration,vin,stock_number").or(`make.ilike.${pattern},model.ilike.${pattern},registration.ilike.${pattern},vin.ilike.${pattern},stock_number.ilike.${pattern}`).limit(20),db.from("crm_reservations").select("*,customer:crm_customers(id,first_name,last_name),bike:stock_bikes(id,make,model,registration)").or(`notes.ilike.${pattern}`).limit(20),db.from("crm_sales").select("id,status,customer:crm_customers(first_name,last_name),bike:stock_bikes!crm_sales_stock_bike_id_fkey(make,model)").limit(20)]);return {customers:(customers.data??[]) as CrmCustomer[],leads:(leads.data??[]) as unknown as CrmLead[],stock:(stock.data??[]) as {id:string;make:string|null;model:string|null;registration:string|null;vin:string|null;stock_number:string|null}[],reservations:(reservations.data??[]) as unknown as CrmReservation[],sales:(sales.data??[]) as unknown as {id:string;status:string;customer?:{first_name:string;last_name:string}|null;bike?:{make:string|null;model:string|null}|null}[]}}
