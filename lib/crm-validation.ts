...
export const stockId=(value:unknown)=>{if(typeof value==="number")return Number.isSafeInteger(value)&&value>0?value:null;const text=cleanText(value,30);return /^\d+$/.test(text)&&Number(text)>0?Number(text):null};
