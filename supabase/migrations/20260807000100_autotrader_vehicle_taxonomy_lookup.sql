alter table public.stock_bikes
  ...
  add column if not exists autotrader_mot_data jsonb not null default '{}'::jsonb;
...
alter table public.stock_bikes
  ...
  drop constraint if exists stock_bikes_autotrader_mot_data_is_object,
  add constraint stock_bikes_autotrader_mot_data_is_object check (jsonb_typeof(autotrader_mot_data) = 'object');
...
alter table public.retail_checks
  ...
  add column if not exists "Auto Trader MOT Data" jsonb not null default '{}'::jsonb;
...
alter table public.retail_checks
  ...
  drop constraint if exists retail_checks_autotrader_mot_data_is_object,
  add constraint retail_checks_autotrader_mot_data_is_object check (jsonb_typeof("Auto Trader MOT Data") = 'object');
